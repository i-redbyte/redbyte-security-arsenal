...
static int analyze_fat_binary(FILE *file, MachOFile *mach_o_file);
...
int analyze_mach_o(FILE *file, MachOFile *mach_o_file) {
    ...
    if (magic == FAT_CIGAM || magic == FAT_MAGIC) {
        // Реализуем поддержку Fat Binaries
        if (analyze_fat_binary(file, mach_o_file) != 0) {
            fprintf(stderr, "Failed to analyze Fat Binary.\n");
            return -1;
        }
    } else {
...
static int analyze_fat_binary(FILE *file, MachOFile *mach_o_file) {
    ...
    if (fread(fatArchs, sizeof(struct fat_arch), nfat_arch, file) != nfat_arch) {
        fprintf(stderr, "Failed to read fat_arch structures.\n");
        free(fatArchs);
        return -1;
    }
    ...
    for (uint32_t i = 0; i < nfat_arch; i++) {
        ...
        uint32_t cputype = OSSwapBigToHostInt32(fatArchs[i].cputype);
        uint32_t cpusubtype = OSSwapBigToHostInt32(fatArchs[i].cpusubtype);
        ...
        // Создаем новую структуру MachOFile для каждой архитектуры
...
static int analyze_load_commands(FILE *file, MachOFile *mach_o_file) {
    uint32_t ncmds;
    uint32_t sizeofcmds;
    uint32_t commands_offset = ftell(file);
    ...
    if (fread(mach_o_file->commands, 1, sizeofcmds, file) != sizeofcmds) {
        fprintf(stderr, "Failed to read load commands.\n");
